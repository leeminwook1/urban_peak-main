  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#1a1a1a]">굿즈 관리</h1>
        <div className="flex gap-2">
          <button onClick={() => { setEditingSectionId(null); setSectionForm({ ...EMPTY_SECTION, display_order: sections.length }); setShowSectionForm(true); }}
            className="flex items-center gap-2 rounded-full border border-[#1a1a2e] px-4 py-2.5 text-sm font-medium text-[#1a1a2e] transition-opacity hover:opacity-70">
          ...
          <button onClick={() => { setEditingItemId(null); setItemForm({ ...EMPTY_ITEM, display_order: items.length }); setItemImages([]); setShowItemForm(true); }}
            className="flex items-center gap-2 rounded-full bg-[#1a1a2e] px-4 py-2.5 text-sm font-medium text-white transition-opacity hover:opacity-80">
            <Plus size={14} /> 상품 추가
          </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4">
          <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-8 shadow-xl">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-lg font-bold text-[#1a1a1a]">{editingSectionId !== null ? "배너 수정" : "새 배너"}</h2>
              <button onClick={() => setShowSectionForm(false)} className="text-[#8a8a8a] hover:text-[#1a1a1a]"><X size={20} /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="mb-1 block text-xs font-medium text-[#6b6b6b]">제목 (큰 텍스트)</label>
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-[#6b6b6b]">서브타이틀 (선택)</label>
              ...
              <div>
                ...
                {sectionForm.banner_image && (
                  <div className="mt-2 flex items-center gap-2">
                    <img src={sectionForm.banner_image} alt="preview" className="h-16 w-32 rounded-lg object-cover" />
            ...
            <div className="mt-6 flex gap-3">
              <button onClick={handleSaveSection} disabled={saving} className="flex flex-1 items-center justify-center gap-2 rounded-full bg-[#1a1a2e] py-3 text-sm font-medium text-white disabled:opacity-50 hover:opacity-80">
                <Check size={15} />{saving ? "저장 중..." : "저장"}
              </button>
              <button onClick={() => setShowSectionForm(false)} className="flex-1 rounded-full border border-[#e5e0d8] py-3 text-sm font-medium text-[#6b6b6b] hover:border-[#1a1a1a]">취소</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showItemForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-4">
          <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-8 shadow-xl">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-lg font-bold text-[#1a1a1a]">{editingItemId !== null ? "상품 수정" : "새 상품"}</h2>
              <button onClick={() => setShowItemForm(false)} className="text-[#8a8a8a] hover:text-[#1a1a1a]"><X size={20} /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="mb-1 block text-xs font-medium text-[#6b6b6b]">상품명</label>
                <input type="text" value={itemForm.name} onChange={e => setItemForm(f => ({ ...f, name: e.target.value }))}
                  className="w-full rounded-xl border border-[#e5e0d8] bg-[#faf8f5] px-4 py-2.5 text-sm outline-none focus:border-[#1a1a2e]" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-[#6b6b6b]">설명</label>
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-[#6b6b6b]">섹션 (배너)</label>
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-[#6b6b6b]">상품 이미지 (최대 4장)</label>
                  ...
                  <div className="mt-2 grid grid-cols-4 gap-2">
                    {itemImages.map((url, idx) => (
                      <div key={idx} className="relative">
                        <img src={url} alt={`img ${idx}`} className="h-20 w-full rounded-lg object-cover" />
                ...
                <p className="mt-1 text-[11px] text-[#a0a0a0]">첫 번째 이미지가 대표 이미지로 사용됩니다. ({itemImages.length}/4)</p>
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-[#6b6b6b]">순서</label>
            ...
            <div className="mt-6 flex gap-3">
              <button onClick={handleSaveItem} disabled={saving} className="flex flex-1 items-center justify-center gap-2 rounded-full bg-[#1a1a2e] py-3 text-sm font-medium text-white disabled:opacity-50 hover:opacity-80">
                <Check size={15} />{saving ? "저장 중..." : "저장"}
              </button>
              <button onClick={() => setShowItemForm(false)} className="flex-1 rounded-full border border-[#e5e0d8] py-3 text-sm font-medium text-[#6b6b6b] hover:border-[#1a1a1a]">취소</button>
            </div>
          </div>
        </div>
      )}
      ...
      {loading ? <p className="text-sm text-[#8a8a8a]">불러오는 중...</p> : (
        <div className="space-y-4">
          {sections.length === 0 && items.length === 0 && (
            <p className="text-sm text-[#8a8a8a]">등록된 배너와 상품이 없습니다. 배너를 먼저 추가하세요.</p>
          )}

          {/* 섹션별 목록 */}
          {sections.map(section => (
            <div key={section.id} className="rounded-2xl bg-white shadow-sm overflow-hidden">
              {/* 배너 헤더 */}
              <div className="flex items-center justify-between px-6 py-4 border-b border-[#f0ebe3]">
                <div className="flex items-center gap-3">
                  {section.banner_image && (
                    <img src={section.banner_image} alt={section.title} className="h-10 w-16 rounded-lg object-cover" />
                  )}
                  <div>
                    <p className="font-bold text-[#1a1a1a]">{section.title}</p>
                    {section.subtitle && <p className="text-xs text-[#8a8a8a]">{section.subtitle}</p>}
                    <p className="text-xs text-[#a0a0a0]">상품 {itemsBySection(section.id).length}개</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => { setEditingSectionId(section.id); setSectionForm({ title: section.title, subtitle: section.subtitle ?? "", banner_image: section.banner_image ?? "", display_order: section.display_order }); setShowSectionForm(true); }}
                    className="rounded-xl border border-[#e5e0d8] p-2 text-[#6b6b6b] hover:border-[#1a1a2e] hover:text-[#1a1a2e]"><Pencil size={14} /></button>
                  <button onClick={() => handleDeleteSection(section.id)}
                    className="rounded-xl border border-[#e5e0d8] p-2 text-[#6b6b6b] hover:border-red-400 hover:text-red-500"><Trash2 size={14} /></button>
                  <button onClick={() => toggleSection(section.id)}
                    className="rounded-xl border border-[#e5e0d8] p-2 text-[#6b6b6b]">
              ...
              {expandedSections.has(section.id) && (
                <div className="p-4">
                  {itemsBySection(section.id).length === 0 ? (
                    <p className="text-center text-xs text-[#a0a0a0] py-4">이 섹션에 상품이 없습니다.</p>
                  ) : (
                    <div className="grid grid-cols-3 gap-3">
                      {itemsBySection(section.id).map(item => (
                        <div key={item.id} className="rounded-xl border border-[#f0ebe3] overflow-hidden">
                          {item.image_url ? (
                            <div className="relative h-32">
                              <Image src={item.image_url} alt={item.name} fill className="object-cover" />
                            </div>
                          ) : (
                            <div className="h-32 bg-[#f0ebe3] flex items-center justify-center">
                              <span className="text-xs text-[#a0a0a0]">이미지 없음</span>
                            </div>
                          )}
                          <div className="p-3">
                            ...
                            <div className="mt-2 flex gap-1">
                              <button onClick={() => { setEditingItemId(item.id); setItemForm({ name: item.name, description: item.description ?? "", image_url: item.image_url ?? "", images: item.images ?? "[]", section_id: item.section_id, display_order: item.display_order }); try { setItemImages(JSON.parse(item.images || "[]")); } catch { setItemImages([]); } setShowItemForm(true); }}
                                className="flex-1 rounded-lg border border-[#e5e0d8] py-1 text-[10px] text-[#6b6b6b] hover:border-[#1a1a2e]">수정</button>
                              <button onClick={() => handleDeleteItem(item.id)}
                                className="flex-1 rounded-lg border border-[#e5e0d8] py-1 text-[10px] text-[#6b6b6b] hover:border-red-400 hover:text-red-500">삭제</button>
                            </div>
                          </div>
                        </div>
                      ))}
                  ...
                  <button onClick={() => { setEditingItemId(null); setItemForm({ ...EMPTY_ITEM, section_id: section.id, display_order: itemsBySection(section.id).length }); setItemImages([]); setShowItemForm(true); }}
                    className="mt-3 flex w-full items-center justify-center gap-1 rounded-xl border border-dashed border-[#e5e0d8] py-2.5 text-xs text-[#a0a0a0] hover:border-[#1a1a2e] hover:text-[#1a1a2e]">
          ...
          {unsortedItems.length > 0 && (
            <div className="rounded-2xl bg-white shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-[#f0ebe3]">
                <p className="font-bold text-[#1a1a1a]">섹션 미지정 상품</p>
              </div>
